...
R dilinde istatistiksel dağıtımlarla (Normal, Binom, Poisson vb.) çalışırken kullanılan fonksiyonların başına gelen **d-p-q-r** harfleri, o dağılımın hangi özelliğini hesaplamak istediğinizi belirtir.

Bu harfler İngilizce terimlerin baş harflerinden gelir: **D**ensity, **P**robability (Cumulative), **Q**uantile, **R**andom.

En yaygın örnek olan **Normal Dağılım (`norm`)** üzerinden bu 4 öneki açıklayalım.
...
-   
    ...
    ![Image of standard normal distribution curve](https://encrypted-tbn3.gstatic.com/licensed-image?q=tbn:ANd9GcRcL-YZeRio-RdG381ETMZn6U1PgK5DJ_pB76TSoWud4URWVG3dtMeowjuriDPw0o8nmz2IwyswNh1jrlArbG_8TTUq_nAiv1JXj0gIBB0Z_7V2bgU)

Örnek (Normal Dağılım):

Standart normal dağılımın (ortalama=0, ss=1) tam tepe noktasındaki (0 noktası) yüksekliği nedir?
...
Örnek (Binom Dağılımı):

Bir madeni parayı 10 kere attığımda, tam olarak 5 kere tura gelme olasılığı nedir?
...
Bu fonksiyon, **eğrinin sol tarafında kalan alanı** verir. İstatistikteki $P(X \le x)$ değeridir.

Bir değerin (veya o değerden küçüklerin) gerçekleşme ihtimalini hesaplarken kullanılır. "X değeri 2'den küçük veya eşit olma olasılığı nedir?" sorusunun cevabıdır.

![](https://encrypted-tbn0.gstatic.com/licensed-image?q=tbn:ANd9GcSKM_B_uz9SkYDPxsXjN-rL7sKmn0cbro7SVTxBcnksVTXlQccScmv9jaYYSjKSlN51n2ny8ce7lFckafCyPYtUqF00wYqxouQdHETJ4o9zGAp6OcI){alt="Image of cumulative distribution function area under curve"}

Örnek:

Standart normal dağılımda bir değerin 0'dan küçük olma ihtimali nedir? (Simetrik olduğu için %50 olmasını bekleriz).
...
Z-skoru 1.96 olan değerin solunda kalan alan (kümülatif olasılık) nedir?
...
Bu fonksiyon, `p` fonksiyonunun **tam tersidir** (Ters Kümülatif Dağılım Fonksiyonu).

Siz R'a bir olasılık (alan) verirsiniz, R size bu olasılığa denk gelen **x sınır değerini** (kesme noktasını) verir. "Hangi değerin altında verilerin %95'i kalır?" sorusunun cevabıdır.

Örnek:

Verilerin en düşük %97.5'ini kapsayan sınır değeri (z-skoru) nedir?

R

```         
qnorm(0.975)
# Sonuç: 1.9599... (Yaklaşık 1.96)
```

Örnek:

Sınıfın en başarılı %10'una girmek için (yani %90'ından daha yüksek not almak için) notum kaç olmalı? (Ortalama 60, SS 10 olsun).
...
Ortalaması 100, standart sapması 15 olan (IQ testi gibi) bir dağılımdan rastgele 5 kişi seç (simüle et).

R

```         
rnorm(n = 5, mean = 100, sd = 15)
# Sonuç Örneği: 102.4, 98.1, 115.3, 89.5, 108.2
```
