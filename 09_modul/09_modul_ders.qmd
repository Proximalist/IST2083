...
# ⚠️ Kritik Ayrım: Güven Aralığı vs. Tahmin Aralığı
...
pi <- predict(model, interval = "prediction", level = 0.95)[1, ]
...
cat("Tahmin Aralığı (Sıradaki öğrenci kaç alır?): [", round(pi["lwr"], 2), "-", round(pi["upr"], 2), "]\n")
```
