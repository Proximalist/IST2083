...
## Bir de Anlatı Olarak Grafik: Minard'ın 1812 Haritası

Anscombe dörtlüsü grafiğin **istatistiksel** gücünü gösterir: aynı sayılar, farklı gerçekler saklayabilir. Grafiğin bir de **anlatısal** gücü vardır ve bunun en çok anılan örneği mühendis Charles Joseph Minard'ın 1869'da çizdiği, Napolyon'un 1812 Rusya seferini anlatan haritasıdır. İstatistikçi Edward Tufte bu haritayı "bugüne kadar çizilmiş en iyi istatistiksel grafik" olarak tanımlar.

Minard'ın orijinal haritasını taşıyan bant, tek bir çizgide **altı değişkeni** birleştirir: ordunun coğrafi konumu (boylam-enlem), asker sayısı (bandın kalınlığı), yön (gidiş/dönüş, renk), zaman (şehir ve tarih etiketleri) ve — ayrı bir panelde — sıcaklık. Haritayı bizzat yeniden üretmeyeceğiz (1869 tarihli bir sanat eserinin taranmış görüntüsünü basmak yerine, altındaki veriyi kullanıp **kendi** grafiğimizi çizeceğiz); ama bu verinin dijitalleştirilmiş hâli (Wilkinson, *The Grammar of Graphics*'in ekleri için, doksanlarda) herkese açıktır ve bu tür bir grafiği ggplot2'de üretmenin kendisi literatürde "Minard'ın Meydan Okuması" (Friendly, 2002) diye anılan klasik bir alıştırmadır:

```{r}
#| fig-height: 5.4
library(patchwork)

troops <- read.csv(here("data", "Minard.troops.csv"))
cities <- read.csv(here("data", "Minard.cities.csv"))
sicaklik <- read.csv(here("data", "Minard.temp.csv"))

ust <- ggplot(troops, aes(x = long, y = lat)) +
  geom_path(aes(linewidth = survivors, color = direction, group = group),
            lineend = "round", linejoin = "round") +
  geom_text(data = cities, aes(label = city), size = 2.6, fontface = "italic") +
  scale_linewidth(range = c(0.5, 10), guide = "none") +
  scale_color_manual(values = c(A = "#DFC17E", R = "#252523"),
                     labels = c("Gidiş", "Dönüş")) +
  coord_cartesian(xlim = c(24, 38)) +
  labs(x = NULL, y = "Enlem", color = NULL) +
  theme_minimal() +
  theme(legend.position = c(0.9, 0.15))

alt <- ggplot(sicaklik, aes(x = long, y = temp)) +
  geom_line(color = "grey40") +
  geom_point(size = 1.5) +
  geom_text(aes(label = paste0(date, ": ", temp, "°")), size = 2.4, vjust = -0.8) +
  coord_cartesian(xlim = c(24, 38)) +
  labs(x = "Boylam", y = "Sıcaklık (°C)") +
  theme_minimal()

ust / alt + plot_layout(heights = c(3, 1))
```

Bandın genişliği, 340.000 kişiyle yola çıkan ordunun Moskova'ya varana kadar nasıl inceldiğini, dönüş yolunda ise neredeyse tümüyle yok olduğunu gösteriyor: haritanın en sağındaki kalın altın rengi bant ile en solundaki ince siyah çizgi arasındaki fark, aynı ordunun başı ve sonu. Alttaki panel bu erimenin **neden** hızlandığını açıklıyor: dönüş yolunda sıcaklık Kasım ortasından itibaren eksi 20-30 dereceye düşüyor. Minard, Napolyon'un adını haritaya hiç yazmadan, yalnızca askerlerin kaderini çizerek bir savaşın anlatısını veriye dönüştürmüş.

::: callout-note
## Kaynak

Veri, Minard'ın haritasının Howard Wilkinson tarafından dijitalleştirilmiş hâlidir ve R'da `HistData` paketiyle (`Minard.troops`, `Minard.cities`, `Minard.temp`) dağıtılır; burada üç küçük CSV olarak `data/` klasörüne eklendi. Bkz. Friendly, M. (2002). Visions and re-visions of Charles Joseph Minard. *Journal of Educational and Behavioral Statistics*, 27(1), 31–51.
:::
...
::: callout-tip
## Araştırmacının alet çantası: `ggrepel` ile gözlemi kimliklendirmek

Bu grafiğe bakan bir araştırmacının ilk sorusu genelde şudur: "peki eğrinin epey altında/üstünde kalan şu birkaç ülke hangisi?" `geom_text()` bu soruyu yanıtlar ama etiketleri noktaların **tam üstüne** yazar; birbirine yakın birkaç ülke varsa etiketler üst üste biner. `ggrepel` paketinin `geom_text_repel()` fonksiyonu aynı işi yapar, ama etiketleri örtüşmeyecek şekilde iter ve gerekirse noktayla arasına ince bir çizgi çizer:

```{r}
#| fig-height: 4
library(ggrepel)

secili_ulkeler <- c("Botswana", "South Africa", "Swaziland", "Vietnam", "Nicaragua")

ggplot(gapminder_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_point(alpha = 0.4) +
  geom_text_repel(data = filter(gapminder_2007, country %in% secili_ulkeler),
                  aes(label = country), size = 3.2, color = "firebrick") +
  geom_point(data = filter(gapminder_2007, country %in% secili_ulkeler),
             color = "firebrick") +
  scale_x_log10() +
  labs(x = "Kişi başı gelir (log ölçek)", y = "Yaşam beklentisi (yıl)")
```

Botswana ve Güney Afrika, gelirlerine göre **beklenenin çok altında** bir yaşam beklentisine sahip; ikisi de 2007'de HIV/AIDS salgınından en çok etkilenen ülkeler arasındaydı. Vietnam ise tam tersi yönde bir aykırı değer: geliri düşük olmasına rağmen yaşam beklentisi beklenenin oldukça üstünde. Bir dağılım grafiğinde aykırı noktaları **kimliklendirmek**, genel eğilimi açıklayan teoriyi test etmenin ilk adımıdır — 10. bölümdeki Simpson paradoksu örneğinde de benzer bir "neden bu noktalar farklı" sorusuyla karşılaşacağız.
:::
...
**Tema**, verinin kendisiyle ilgisi olmayan her şeyi yönetir: arka plan, ızgara çizgileri, yazı tipleri, lejantın konumu. Hazır temalar tek satırda görünümü değiştirir: `theme_minimal()`, `theme_bw()`, `theme_classic()`. Tek tek ayarlar `theme()` ile yapılır, örneğin `theme(legend.position = "bottom")`. Bir yayın veya politika raporu için hazırlıyorsanız, `ggthemes` paketindeki `theme_economist()` gibi hazır gazetecilik temaları da bir seçenektir — kavramsal olarak yeni bir şey öğretmez, yalnızca aynı `theme_*()` mantığını farklı bir pakette kullanır.
...
| *Ufuk turu —* aktörler/ülkeler arası ilişkiler nasıl? | ağ veya akış verisi | Ağ diyagramı, Sankey/chord | `ggraph`, `ggalluvial` *(bu modülde işlenmedi)* |
...
Son satır bu dersin **dışında**: ticaret hacimleri, ittifak ağları veya göç akışları gibi "kim kiminle" sorularını gösteren ağ ve akış diyagramları, kendi başına bir dilbilgisi (`ggraph`'ın kendi `geom_edge_*()` ailesi) gerektirir. Uluslararası ilişkiler araştırmasında sık karşılaşacağınız için adını buraya not düşüyoruz; nasıl çizileceği bu dersin kapsamının ötesinde.
